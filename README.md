...
- `IMU_Driver/LoopTester.py` - Calls 'createFakePacket' from `fake_data_extern.cpp` (after it is built into a .so library file) to create a fake packets, sends them to a specified port. Also opens a server socket and receives any broadcasted data on port 8888 and verifies whether it matches fields in the fake packet
- `IMU_Driver/DummyReceiver.py` - Receives broadcasted messages and prints them. Good for verification purposes or to simulate a client process.
...
- Runs `./parser_test` to execute 5 unit tests to test the C++ parsing function
...
## What the LoopTester.py run looks like
![Alt text](./ExampleTestView.png)
